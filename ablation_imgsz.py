"""
imgsz 消融实验脚本
对比不同分辨率 (640/960/1280/1600) 对 mAP 的影响
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='imgsz 消融实验')
    parser.add_argument('--weights', type=str, required=True,
                        help='模型权重路径')
    parser.add_argument('--data', type=str, 
                        default='./data/deeppcb/yolo_format',
                        help='数据集路径')
    parser.add_argument('--imgsz-list', type=int, nargs='+',
                        default=[640, 960, 1280, 1600],
                        help='要测试的分辨率列表')
...
def evaluate_imgsz(model, img_dir, label_dir, imgsz, conf, iou, device):
    """在指定分辨率下评估模型"""
    ...
    for img_path in tqdm(image_files, desc=f"    imgsz={imgsz}", leave=False):
...
def main():
    ...
    print("消融实验结果汇总")
    print("="*70)
    print(f"{'imgsz':>8} | {'mAP@0.5':>10} | {'mAP@0.5:0.95':>12} | {'FPS':>8}")
    print("-"*70)
    for r in results:
        print(f"{r['imgsz']:>8} | {r['map50']*100:>9.2f}% | {r['map50_95']*100:>11.2f}% | {r['fps']:>8.1f}")
    print("="*70)
    ...
    for r in results:
        row = f"{r['imgsz']:>8}"
        for name in CLASS_NAMES:
            row += f" | {r['per_class_ap50_95'][name]*100:>7.2f}%"
        print(row)
    
    # 保存JSON
    if args.save_json:
        output_file = 'imgsz_ablation_results.json'
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump({
                'experiment': 'imgsz ablation',
                'model': args.weights,
                'data': args.data,
                'results': results
